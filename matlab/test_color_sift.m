...
if do_svm_linar_classification
    ...
    C_vals=log2space(7,10,5);
    for i=1:length(C_vals);
        opt_string=['-t 0  -v 4 -c ' num2str(C_vals(i))];
...
if %do_svm_chi2_classification    
    ...
    n_train = size(bof_train, 1);
    n_test = size(bof_test, 1);
    
    % Costruzione della matrice del kernel precomputato per l'addestramento
    Ktrain = [(1:n_train)', kernel_expchi2(bof_train, bof_train)];
    
    % Costruzione della matrice del kernel precomputato per il test
