function [ ] = visualize_results_pyr( classes, desc_test, labels_test, labels_result )
...
for i=1:length(classes)
    ...
    if length(indcorr)
        for j=1:length(indcorr) 
            pattern = '_\d';
            replace = '';
            d = desc_test(indcorr(j));
            new_name = regexprep(d.imgfname,pattern,replace);
            imgcorr{end+1}=imread(new_name);
        end
